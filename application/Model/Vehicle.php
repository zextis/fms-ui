...
class Vehicle extends Model
{
    public function getAllVehicles() {
        ...
        $sql = "SELECT vehicles.`license_plate`, vehicles.`vehicle_type`, vehicles.`body_type`, vehicles.`make`, vehicles.`model`, vehicles.`year`, vehicles.`transmission`, vehicles.`fuel`, vehicles.`production_year`, facilities.name AS facility, vehicles.`engine_number`, vehicles.`chasis_number`, vehicles.`colour`, vehicles.`seating`, vehicles.`cc_rating`, vehicles.`fitness_expiration`, vehicles.`license_expiration`, vehicles.`next_maintenance`, vehicles.`is_available`, vehicles.`is_operable`, vehicles.`created_at`, vehicles.`updated_at` FROM `vehicles` INNER JOIN facilities on facility_id = facilities.id";
    ...
    public function getVehicle($id){
        $sql = "SELECT vehicles.`license_plate`, vehicles.`vehicle_type`, vehicles.`body_type`, vehicles.`make`, vehicles.`model`, vehicles.`year`, vehicles.`transmission`, vehicles.`fuel`, vehicles.`production_year`, facility_id, vehicles.`engine_number`, vehicles.`chasis_number`, vehicles.`colour`, vehicles.`seating`, vehicles.`cc_rating`, vehicles.`fitness_expiration`, vehicles.`license_expiration`, vehicles.`next_maintenance`, vehicles.`is_available`, vehicles.`is_operable`, vehicles.`created_at`, vehicles.`updated_at` FROM `vehicles` WHERE license_plate = :id LIMIT 1";
        $query = $this->db->prepare($sql);
    ...
    {
        $sql = "UPDATE `vehicles` SET `license_plate`=:license_plate,`vehicle_type`=:vehicle_type,`body_type`=:body_type,`make`=:make,`model`=:model,`year`=:year,`transmission`=:transmission,`fuel`=:fuel,`production_year`=:production_year,`facility_id`=:facility_id,`engine_number`=:engine_number,`chasis_number`=:chasis_number,`colour`=:colour,`seating`=:seating,`cc_rating`=:cc_rating,`fitness_expiration`=:fitness_expiration,`license_expiration`=:license_expiration,`next_maintenance`=:next_maintenance,`is_available`=:is_available,`is_operable`=:is_operable,`created_at`=:created_at,`updated_at`=:updated_at WHERE license_plate = :id";

        $query = $this->db->prepare($sql);
        
        $current_date = date("Y-m-d H:i:s"); 
        $parameters = array(':id' => $id, ':license_plate' => $license_plate, ':vehicle_type' => $vehicle_type, ':body_type' => $body_type, ':make' => $make, ':model' => $model, ':year' => $year, ':transmission' => $transmission, ':fuel' => $fuel, ':production_year' => $production_year, ':facility_id' => $facility_id, ':engine_number' => $engine_number, ':chasis_number' => $chasis_number, ':colour' => $colour, ':seating' => $seating, ':cc_rating' => $cc_rating, ':fitness_expiration' => $fitness_expiration, ':license_expiration' => $license_expiration, ':next_maintenance' => $next_maintenance, ':is_available' => $is_available, ':is_operable' => $is_operable, ':created_at' => $current_date, ':updated_at' => $current_date);
